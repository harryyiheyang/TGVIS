...
```{r,echo=F}
filepath="C:/Users/yxy1234/Downloads/TGVIS_New/"
```
...
# residual_variance is the inflation factor
fit1$Summary
```
...
## TGVIS with Adaptively Estimated Infinitesimal Effects
TGVIS also supports adaptive estimation of infinitesimal effects using variants within a locus. Although this may require additional computational time, it serves as a useful alternative to fixing the variance of the infinitesimal effect at the per-SNP heritability estimate obtained from LDSC.

```{r}
t1 <- Sys.time()
fit2=tgvis(estimate_inf=T,by=by,bXest=bXest,LD=R,L_vec=c(7:8),
          Noutcome=mean(LDL_Locus$N)) 
# Set small number of candidate L to reduce the computational time
# Set estimate_inf=T to estimate the infinitesimal effect adaptively
# Please remove var_inf and residual_variance two parameters.
t2 <- Sys.time()
print(difftime(t2, t1, units = "secs"))
fit2$Summary
```
In our experience, setting `estimate_inf = TRUE` can be computationally intensive for loci with strong genetic associations, such as the PCSK9 locus for LDL-C and the CETP locus for HDL-C. Importantly, however, the choice between `estimate_inf = TRUE` and `FALSE` has negligible impact on results at such loci, because the dominant fine-mapping signals overwhelm any infinitesimal effect. For loci with weak or moderate genetic associations, the running time is typically within 10 minutes, and the two settings may yield different results. We therefore recommend running both as a robustness check for weaker loci.
