tgvis_fast=function(by,bXest,LD,Lvec=c(1:8),n_outcome,var_inf=0.15/1e6,eigen_thres=0.999,pip.thres.cred=0.001,susie_iter=500,estimate_residual_variance=F,scaled_prior_variance=0.5,residual_variance=1,prior.weight.theta=NULL,prior.weight.gamma=NULL,ebic.factor=1){
  ######################### basic infromation ####################################
  m=length(by)
  p=ncol(bXest)
  ...
  for(i in 1:length(Lvec)){
    ...
    Bicvec[i]=log(rss)+(log(idx)+log(m+p)*ebic.factor)/idx*Lvec[i]
  }
